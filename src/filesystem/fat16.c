...
void* fat16_open(struct disk* disk, struct path_part* path, FILE_MODE mode)
{
    ...
    desc->item = get_fat16_dir_entry(disk, path);
    if (desc->item == NULL)
    {
        kernel_free(desc);
        return ERROR(-MYOS_ERROR_NO_MEMORY);
...
int fat16_read_internal(struct disk* disk, int cluster, int offset, int size, void* out_buffer)
{
    ...
    int res = 0;
    struct fat_private* fat_private = disk->fs_private_data;
    struct streamer* streamer = fat_private->cluster_read_stream;

    int cluster_size = fat16_get_cluster_size(fat_private);
...
//sub-directory load
struct fat_directort* fat16_load_fat_sub_directory(struct disk* disk, struct fat_directory_item* item)
{
    int res = 0;
    ...
    int actual_data_size = item->filesize;
out:
}

struct fat_directory* fat16_load_fat_directory(struct disk* disk, struct streamer* streamer, struct fat_directory_item* item)
{
    int res = 0;
    struct fat_directory *dir;
    struct fat_private* fat_private = disk->fs_private_data;
    ...
    //1. get first cluster
    //cluster 안에는 file datas, fat 16은 어떤 클러스터를 사용할지 map 지도
    int cluster = fat16_get_first_cluster(item);
    int cluster_sector = fat16_get_sector_from_cluster(fat_private, cluster);
    int total_item = fat16_get_total_items_for_dir(disk, cluster_sector);
    dir->total = total_item;
    int dir_size = dir->total * sizeof(struct fat_directory_item);
    ...
    /*
        in file allocation table, we have fat16 table.
        there's number to point next cluster.
    */
...
struct fat_item *fat16_new_item_from_directory_item(struct disk *disk, struct fat_directory_item *directory_item)
{
    ...
    if (directory_item->attribute & FAT_FILE_SUBDIRECTORY)
    {
        ...
        f_item->directory = fat16_load_directory(disk, directory_item);
    }
    else
    {
...
struct fat_item *fat16_find_item_in_dir(struct disk *disk, struct fat_directory *directory, const char *name)
{
    ...
    while(i < directory->total)
    {
        ...
        //if name is matched
        if (istrncmp(tempfile, name, ft_strlen(name)) == 0)
        {
